...
const ProgramOutcomesList: React.FC = () => {
  ...
  const handleExcelUpload = (uploadedData: { number: string; description: string }[]) => {
    if (!selectedProgram || !setData) return;
  ...
  const handleDeletePo = (poId: string) => {
    if (window.confirm("Are you sure you want to delete this Program Outcome?") && setData) {
        // TODO: Replace with API call
        setData(prev => prev ? ({
            ...prev,
  ...
  if (!selectedProgram || !data) {
    return (
        <div className="text-center p-8 bg-white rounded-lg shadow-md">
            <h1 className="text-2xl font-bold text-gray-700">Program Outcomes</h1>
            <p className="text-gray-500 mt-2">Please select a program to manage its outcomes.</p>
        </div>
    )
  }
