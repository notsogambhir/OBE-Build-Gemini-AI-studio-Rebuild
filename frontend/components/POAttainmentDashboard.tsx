...
import { ProgramOutcome, AppData } from '../types';
...
const POAttainmentDashboard: React.FC<POAttainmentDashboardProps> = ({
  programOutcomes,
  draftState,
  onStateChange,
  data,
  ...
  const directAttainment = useMemo(() => {
    ...
    programOutcomes.forEach((po) => {
      const relevantMappings = data.coPoMapping.filter((m) => m.poId === po.id);
      let weightedSum = 0;
      let totalWeight = 0;

      relevantMappings.forEach((mapping) => {
        const co = data.courseOutcomes.find((c) => c.id === mapping.coId);
        if (!co) return;

        const coLevel = calculateOverallCoLevel(co, data);
        weightedSum += coLevel * mapping.level;
        totalWeight += mapping.level;
      });

      const directValue = totalWeight > 0 ? weightedSum / totalWeight : 0;
      attainment[po.id] = parseFloat(directValue.toFixed(2));
    });
    return attainment;
  }, [programOutcomes, data]);

  const handleIndirectChange = useCallback((poId: string, value: string) => {
    onStateChange({
      ...draftState,
      indirectAttainment: {
        ...indirectAttainment,
  ...
  }, [draftState, onStateChange, indirectAttainment]);

  const handleWeightChange = useCallback((type: 'direct' | 'indirect', value: string) => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse border border-gray-300">
          <thead className="bg-gray-50">
            <tr>
              <th className="border border-gray-300 p-2 text-sm font-medium text-gray-500 uppercase">
                Attainment Type
              </th>
              <th className="border border-gray-300 p-2 text-sm font-medium text-gray-500 uppercase">
                Weightage
              ...
              {programOutcomes.map((po) => (
                <th
                  key={po.id}
                  className="border border-gray-300 p-2 text-sm font-medium text-gray-500 uppercase"
                >
                  {po.number}
                </th>
              ))}
          ...
          <tbody>
            <tr className="bg-white">
              ...
              <td className="border border-gray-300 p-1">
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => handleWeightChange('direct', e.target.value)}
              ...
              {programOutcomes.map((po) => (
                <td
                  key={po.id}
                  className="border border-gray-300 p-2 text-center font-semibold text-green-600"
                >
                  {directAttainment[po.id]}
                </td>
              ))}
            </tr>
            <tr className="bg-white">
              ...
              <td className="border border-gray-300 p-1">
                <div className="flex items-center">
                  <input
                    type="number"
                    value={weights.indirect}
                    onChange={(e) => handleWeightChange('indirect', e.target.value)}
                    className="w-24 p-2 bg-white text-gray-900 border border-gray-300 rounded-md text-center focus:ring-indigo-500 focus:border-indigo-500"
                  />
                  <span className="ml-1 text-gray-700">%</span>
                </div>
            ...
            <tr className="bg-gray-100 font-bold">
              ...
              {programOutcomes.map((po) => (
                <td
                  key={po.id}
                  className="border border-gray-300 p-2 text-center text-xl text-yellow-600"
                >
