...
const CoursesList: React.FC = () => {
    ...
    const { activeCourses, futureCourses, completedCourses } = useMemo(() => {
        const courses = data.courses.filter((c) => c.programId === selectedProgram?.id).sort((a, b) => a.code.localeCompare(b.code));
        return {
    ...
    const handleExcelUpload = (uploadedData: { code: string; name: string }[]) => {
        ...
        const newCourses: Course[] = uploadedData.map((row, index) => ({
            ...
            attainmentLevels: { level3: 80, level2: 70, level1: 50 }, status: 'Future',
        }));
    ...
    if (currentUser?.role === 'Teacher') {
        // Teachers see a simplified, non-editable view
        return (
